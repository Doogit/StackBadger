...
| `Profile not found` | `run.sh` `cd`s into the StackBadger repo before resolving `--profile`, so a relative path is resolved from the **repo root**, not from where you invoked it | Use a repo-root-relative path (`profiles/<name>.yaml`) or an absolute path |
| `Unsupported auth adapter` | `stack.auth` is not one of `clerk` / `firebase` / `supabase-auth` / `nextauth` — there is no generic fallback adapter; the run cannot proceed | Set `stack.auth` to a supported provider (or implement an adapter for yours). `PENTEST_USER_*_JWT` does not help here: it is a sign-in fallback *inside* the Clerk adapter, not a substitute adapter |
